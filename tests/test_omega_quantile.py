...
def test_weighted_sample_without_replacement_masks_excludes_zero_probability_sites():
    p = numpy.array([0.7, 0.3, 0.0, 0.0], dtype=numpy.float64)
    ...
    numpy.testing.assert_array_equal(masks.sum(axis=1), numpy.full((128,), 2))


def test_weighted_sample_without_replacement_masks_raises_when_size_exceeds_positive_sites():
    p = numpy.array([0.0, 1.0, 0.0], dtype=numpy.float64)
...
def test_get_permutations_fast_returns_expected_shape_and_bounds():
    cb_ids = numpy.array([[0, 1], [1, 2], [0, 2]], dtype=numpy.int64)
    sub_branches = numpy.array([2, 3, 5], dtype=numpy.int64)
    p = numpy.array([0.2, 0.1, 0.25, 0.15, 0.1, 0.1, 0.05, 0.05], dtype=numpy.float64)
