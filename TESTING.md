...
- `tests/cli/` invokes the command-line entry point in subprocesses.
...
Use the full suite before merging a change:
...
```bash
pytest -q tests/unit tests/cli -m "not slow and not parity"
```
...
compiled extension may use `@pytest.mark.requires_cython`.
