...
configuration is excluded. This compares different exposure definitions, not
equivalent statistical models; unchanged candidate/event columns are checked.
"""
import argparse
import hashlib
import json
...
def command(args, mode, outdir):
    ...
    result = ["csubst", "scan", "--alignment_file", str(data / "PEPC.alignment.fa"),
              ...
              "--scan_match", "any2spe", "--scan_rate_exposure", "endpoint" if mode == "endpoint" else "q_weighted",
              "--scan_rate_length", "n_rescaled" if mode == "legacy_default" else "raw",
              "--scan_pvalue_calibration", args.calibration, "--scan_n_permutations", str(args.niter),
...
def worker(args):
    from csubst import cli, substitution_scan
...
def compare_tables(tables):
    ...
    keep = ["target_event_count", "other_event_count", "support_unit_count", "candidate_event_pp_sum"]
    ...
    for mode, frame in indexed.items():
        pd.testing.assert_frame_equal(reference[keep], frame[keep], check_exact=True)
        a = reference.p_rate_enrichment_asymptotic.rank()
        b = frame.p_rate_enrichment_asymptotic.rank()
        result[mode] = dict(candidates=len(frame), unchanged_observations=True,
                            finite_p=int(np.isfinite(frame.p_rate_enrichment_asymptotic).sum()),
                            median_rate_ratio=float(frame.rate_ratio.median()),
                            p_rank_spearman_vs_legacy=float(a.corr(b)),
                            nominal_p_below_005=int((frame.p_rate_enrichment_asymptotic < .05).sum()),
                            nominal_q_below_005=int((frame.q_rate_enrichment_asymptotic_by_trait_match < .05).sum()))
    return result
...
def main():
    ...
    parser.add_argument("--calibration", choices=["none", "full_scan"], default="none")
    parser.add_argument("--niter", type=int, default=20)
    parser.add_argument("--worker", choices=["legacy_default", "q_raw", "endpoint"])
