...
def test_node_combination_subsamples_rifle_handles_dep_ids_list_without_crashing():
    ...
    g = {
        ...
        "dep_ids": [numpy.array([bid], dtype=numpy.int64) for bid in sub_ids],
    }
    numpy.random.seed(0)
    ...
    assert id_combinations.dtype == numpy.int64
...
def test_node_combination_subsamples_shotgun_returns_2d_empty_when_no_independent_combo():
    ...
    g = {
        "tree": tr,
        "sub_branches": sub_ids,
        "dep_ids": [numpy.array(sub_ids, dtype=numpy.int64)],
    }
...
def test_get_node_combinations_cb_passed_multi_trait_does_not_overprune_between_traits():
    ...
    g = {
        "tree": tr,
        "dep_ids": [numpy.array([bid], dtype=numpy.int64) for bid in [a, b, c, d]],
        "fg_dep_ids": {
            "traitA": [numpy.array([b, c], dtype=numpy.int64)],
            "traitB": [numpy.array([b, d], dtype=numpy.int64)],
        },
...
def test_get_node_combinations_logs_foreground_removal_when_count_is_even(capsys):
    ...
    g = {
        "tree": tr,
        "dep_ids": [numpy.array([bid], dtype=numpy.int64) for bid in [a, b, c, d]],
        "fg_dep_ids": {"traitA": [numpy.array([b, c], dtype=numpy.int64)]},
        "fg_df": pandas.DataFrame({"name": ["A", "B", "C", "D"], "traitA": [1, 1, 1, 1]}),
        "threads": 1,
        "exhaustive_until": 2,
    }
...
def test_nc_matrix2id_combinations_ignores_columns_with_wrong_arity():
    ...
    expected = numpy.array([[0, 1]], dtype=numpy.int64)
